...
const ToastComponent = ({ message }) => {
  ...
  useEffect(() => {
    // Set a timeout to automatically hide the toast after 4 seconds
    const timeout = setTimeout(() => {
      setShowToast(false);
    }, 4000);

    // Clear the timeout when the component unmounts or when showToast becomes false (if the user manually closes the toast)
  ...
  return (
    ...
    //   <button className="toast-close" onClick={handleClose}>Close</button>
    // </div>
    <div id="toast-default" className={`toast flex fixed bottom-3 right-3 items-center w-full max-w-xs p-4 text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800 ${showToast  ? 'show' : 'hide'}`} role="alert">
